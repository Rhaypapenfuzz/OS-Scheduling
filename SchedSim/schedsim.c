...
{ 
   ...
   */
    ...
    while(!is_done){
      is_done = 1; 
      for(i = 0; i < n; i++){
        if (remaining_arr[i] > 0){
          is_done = 0; 

          if (remaining_arr[i] > quantum){
            t = t + quantum;
            remaining_arr[i] -= quantum;
          }
          else{
            t = t + remaining_arr[i];
...
{ 
     ...
     */
      ...
      while(process_counter < n){
        int min_index = 0;
        for(i = 1; i < n; i++){
          if (remaining_arr[i] < remaining_arr[min_index]){
            min_index = i;
          }
        }
        
        remaining_arr[min_index] -= 1;
        
        if (remaining_arr[min_index] == 0){
          process_counter += 1;
          
          completion_time = current_time + 1;
          
          plist[min_index].wt = completion_time - plist[min_index].art - plist[min_index].bt;
          remaining_arr[min_index] = INT_MAX;
        }
      current_time += 1;
      }
} 
...
{ 
	ProcessType first_process = *(ProcessType*)this;
	ProcessType second_process = *(ProcessType*)that;
	if (first_process.pri < second_process.pri){
			return 1;
	} else {
		return 0;
	}
